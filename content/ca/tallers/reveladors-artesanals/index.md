---
title: "Reveladors artesanals"
subtitle: "Formular, barrejar i entendre el que revela les teves imatges"
lead: "Un taller per a qui vol entendre la química del revelat fins al punt de poder preparar el seu propi revelador. La fòrmula Beers, el Caffenol i altres receptes artesanals."
description: "Taller de reveladors fotogràfics artesanals: fòrmula Beers, Caffenol i receptes alternatives. Fotografia analògica avançada a Barcelona."
image: "/images/tallers/reveladors-artesanals.jpg"

blocs:
  - processos-alternatius
  - proces
levels:
  - avançat
formats:
  - curs
tags: ["reveladors", "artesanals", "fòrmules", "química", "Beers", "Caffenol", "DIY"]

duration: "8 hores (2 sessions de 4h)"
sessions: 2
price: 140
max_places: 4
location: "Nau Bostik, Barcelona"
extern: false

objective: "Preparar i avaluar reveladors fotogràfics artesanals, entendre els seus components actius i ser capaç d'adaptar-los a les pròpies necessitats."
methodology: "Laboratori pràctic: es preparen i proven diverses fòrmules, es comparen els resultats i es documenta el procés per a ús personal."
result: "Diverses fòrmules preparades i provades, un registre personal de resultats i la capacitat de formular reveladors adaptats a les pròpies necessitats."
prerequisites: "Saber revelar pel·lícula amb reveladors comercials. Recomanable haver fet el taller de Revelats Experimentals."
target: "Fotògrafs analògics avançats que volen entendre la química del revelat en profunditat i treballar amb fòrmules pròpies."
related:
  - revelats-experimentals
  - revelat-bn
  - cianotipia

status: "soon"
date: 2024-01-01
draft: false
---

## Entendre el que fas servir

La majoria de fotògrafs analògics usen reveladors comercials que surten en pols o en solució concentrada i se segueixen les instruccions del fabricant. Funciona. Però no saber com estan fets és com cuinar sempre amb salses de pot: eficient, però sense saber el que hi ha a dins.

En aquest taller anem a l'arrel. Estudiem els components actius dels reveladors —agents reveladors, acceleradors, conservants, antivelats— i prepararem diverses fòrmules des de zero: la Beers Divided Developer, el Caffenol i algunes variants clàssiques i modernes.

No és un taller de química acadèmica. És un taller molt pràctic on l'objectiu és revelar pel·lícula amb el que hem preparat nosaltres mateixos i entendre per què el resultat és el que és.

## Continguts clau

### Sessió 1 — Química del revelat
- Agents reveladors: metol, hidroquinona, fenidona, vitamina C
- Acceleradors, conservants i antivelats: funció de cada component
- La relació entre fòrmula i resultat: gra, contrast, acutance
- Preparació de la fòrmula Beers Divided Developer: dissolució A i B
- Primera prova: revelar una tira amb Beers a diverses proporcions A:B

### Sessió 2 — Receptes alternatives i documentació
- Caffenol-C: cafè instantani, àcid ascòrbic i carbonat sòdic
- Rodinal casolà: para-aminofenol i hidròxid sòdic
- Variacions sobre el D-76/ID-11
- Comparació de resultats: densitat, gra i to
- Documentació pròpia: com registrar i reproduir els resultats

## Materials inclosos

Tots els productes químics, reactius i materials de laboratori. Pel·lícula de prova inclosa. Si vols treballar amb el teu material personal, porta'l.
