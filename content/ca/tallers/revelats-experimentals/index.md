---
title: "Revelats experimentals"
subtitle: "Fòrmules artesanals, processos forçats i reveladors casolans"
lead: "Més enllà del revelador estàndard: push i pull, stand development, Caffenol, la fòrmula Beers i altres maneres d'alterar el resultat químic de la imatge."
description: "Taller de revelats experimentals: push/pull, stand development, Caffenol, fòrmula Beers i reveladors artesanals. Laboratori fotogràfic a Barcelona."
image: "/images/tallers/revelats-experimentals.jpg"

blocs:
  - proces
  - processos-alternatius
levels:
  - intermedi
  - avançat
formats:
  - curs
tags: ["revelat", "experimental", "push", "pull", "caffenol", "beers", "artesanal", "fòrmules"]

duration: "8 hores (2 sessions de 4h)"
sessions: 2
price: 130
max_places: 4
location: "Nau Bostik, Barcelona"
extern: false

objective: "Entendre com les variables químiques alteren el resultat de la imatge i experimentar amb reveladors alternatius i processos no estàndard."
methodology: "Experimentació directa: es revelen diverses tires amb paràmetres diferents per comparar resultats."
result: "Diverses tires revelades amb processos distints i un registre personal de resultats per a cada tècnica provada."
prerequisites: "Saber revelar pel·lícula en B/N. Cal haver fet el taller de Revelat B/N o tenir experiència equivalent."
target: "Fotògrafs analògics que volen anar més lluny del revelat estàndard i entendre com manipular químicament el resultat."
related:
  - revelat-bn
  - cianotipia
  - reveladors-artesanals

status: "soon"
date: 2024-01-01
draft: false
---

## El revelador no és neutral

El revelador estàndard i els temps recomanats pel fabricant donen un resultat previsible. Però la previsibilitat no és l'única opció. La temperatura, la dilució, el temps, el moviment de l'agitació, els components químics —tot afecta el resultat. I quan entens com, pots decidir cap on vols anar.

En aquest taller treballem diverses tècniques que s'allunyen del procés convencional: el push i el pull del negatiu, el *stand development* (revelat sense agitació), el Caffenol (revelador fet amb cafè), i la fòrmula Beers Divided Developer —una de les receptes artesanals més versàtils per controlar el contrast.

No es tracta de fer rareses per fer-les. Es tracta d'entendre la química per dominar-la.

## Continguts clau

### Sessió 1 — Processos forçats
- Push i pull: subexposició i sobreexposició compensada en revelat
- Efectes sobre el gra, el contrast i la latitud
- Stand development: revelat llarg sense agitació i els seus resultats
- Comparació de tires revelades amb cada mètode

### Sessió 2 — Reveladors artesanals
- Principis actius dels reveladors: metol, hidroquinona, vitamina C
- La fòrmula Beers Divided Developer: preparació i ús
- Caffenol-C: revelador de cafè, àcid ascòrbic i carbonat
- Altres fòrmules: Rodinal casolà, D-76 alternatiu
- Registre de resultats i calibratge personal

## Inclòs en el preu

- Tots els productes experimentals: components per a Caffenol, fòrmula Beers, stand developer, etc.
- Pel·lícula de prova per a les comparatives

## Cal portar

- Carretes propis exposats si vols treballar amb material personal (opcional però recomanat)

## No inclòs

- Carretes addicionals per a proves extra (~5€/unitat)
